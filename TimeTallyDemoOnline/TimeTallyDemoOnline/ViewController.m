...
@interface ViewController ()<TimeLineViewDelegate,ServerOperationsDelegate>
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    //下载数据
    if (![[NSUserDefaults standardUserDefaults] boolForKey:@"isLoaded"]) {
        //从服务器读取数据
        //登录后只读取一次
        //发送服务器请求加载数据
        [[ServerOperations sharedInstance] loadDataFormServer];
    ...
    if (!self.timer.isValid) {
        self.timer = [NSTimer scheduledTimerWithTimeInterval:2 repeats:YES block:^(NSTimer * _Nonnull timer) {
            ...
            NSArray *array = [[CoreDataOperations sharedInstance] getAllTallyWithArray];
            if (array.count == 0) {
                [timer invalidate];
                timer = nil;
                NSLog(@"timetiem end-------");
            }
            [self uploadTallyToServer];
...
- (void)uploadTallyToServer{
    ServerOperations *serverOp = [ServerOperations sharedInstance];
    serverOp.delegate = self;
    [serverOp uploadTallyToServer];
}
...
- (void)readSqliteData{
    ...
    self.timeLineModelsDict = [[CoreDataOperations sharedInstance] getAllDataWithTimeLineModelDict];
}
...
- (void)willDeleteCurrentTallyWithIdentity:(NSString*)identity {
    ...
    [alertVC addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        NSLog(@"---%@---%@",tally.identity,tally.typeship.typename);
        ...
        //从服务器上删除
        [[ServerOperations sharedInstance] deleteTallyInServerWithIdentity:identity];
