...
- (void)addTallySave {
    ...
    if ( ![self.typeLab.text isEqualToString:@""] && [self.nValue doubleValue] != 0) {
        [self clickAdd];
        //新增一条数据并保存
        [[CoreDataOperations sharedInstance] addNewTallyWithTallyType:self.typeLab.text andMoney:[self.resutlStr doubleValue]];
...
- (void)modifyTallySavedWithIdentity:(NSString *)identity {
    ...
    //修改数据
    [[CoreDataOperations sharedInstance] modifyTallySavedWithIdentity:identity andTallyType:self.typeLab.text andMoney:[self.resutlStr doubleValue]];
